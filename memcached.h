...
//返回item中data字段suffix的地址
#define ITEM_suffix(item) ((char*) &((item)->data) + (item)->nkey + 1 \
         + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))

//返回item中data字段value的地址
...
typedef struct _stritem {
    ...
    union {
        uint64_t cas;
        char end;
    } data[];  //数据，这个数据包括key，value，CAS，后缀等等
...
//libevent worker线程结构体
typedef struct {
    pthread_t thread_id;        /* unique ID of this thread */  //线程id
    struct event_base *base;    /* libevent handle this thread uses */ //线程所使用的libevent处理框架对象
    struct event notify_event;  /* listen event for notify pipe */ //管道监听事件
    int notify_receive_fd;      /* receiving end of notify pipe */ //管道接收fd
    int notify_send_fd;         /* sending end of notify pipe */ //管道发送fd
    struct thread_stats stats;  /* Stats generated by this thread */	//线程状态
    struct conn_queue *new_conn_queue; /* queue of new connections to handle */	 //CQ_ITEM队列
    cache_t *suffix_cache;      /* suffix cache */
    uint8_t item_lock_type;     /* use fine-grained or global item lock */  //控制线程锁的粒度
} LIBEVENT_THREAD;
...
struct conn {
    ...
    char   *ritem;  /** when we read in an item's value, it goes here */  //指向item结构体的data中value地址
    ...
    void   *item;     /* for commands set/add/replace  */  //执行set/add/replace命令时，用于指向分配的item空间
    ...
    int    msgsize;   /* number of elements allocated in msglist[] */  //*msglist数组大小
    int    msgused;   /* number of elements used in msglist[] */  //*msglist数组已使用的元素个数
    int    msgcurr;   /* element in msglist[] being transmitted now */  //当前要发送的msghdr
    int    msgbytes;  /* number of bytes in current msg */  //当前msghdr的字节数

    item   **ilist;   /* list of items to write out */  //get key1 key2命令时，要发送给客户端的item列表
    int    isize;  //列表大小
