...
//将item内存置为空闲，以供再使用，这里的free不是把内存空间释放
...
//unlink item， 将item从hash表和lru链表中移除
 ...
 */
 //使用爬虫处理过期的item，这里讲这部分工作放入一个专门的线程来进行
